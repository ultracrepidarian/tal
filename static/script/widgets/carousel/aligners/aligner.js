...
require.def(
    ...
    function(Class, BeforeAlignEvent, AfterAlignEvent, AlignmentQueue, RuntimeContext) {
        'use strict';
        /**
            ...
            _align: function(navigator, direction, options) {
                ...
                if (targetIndex !== null) {
                    ...
                    if (this._isWrap(startIndex, targetIndex, direction)) {
                        ...
                        var widget = this._mask.getWidgetStrip();
                        if (targetIndex === 1) {
                            this._animation = RuntimeContext.getDevice().moveElementTo({
                                el: widget.outputElement,
                                to: {
                                    left: 0
                                },
                                duration: options.duraton,
                            ...
                            //     el: widget.outputElement,
                            //     to: {
                            //         left: parseInt(widget.outputElement.style.left.replace(/px/g, '')) - 42 + 'px'
                            //     },
                            //     duration: options.duraton,
                            //     easing: options.easing,
                            //     skipAnim: true
                            // })
                        } else if (targetIndex > 1 && targetIndex < 10) {
                            // if (this._animation) {
                            //     RuntimeContext.getDevice().stopAnimation(this._animation);
                            // }
